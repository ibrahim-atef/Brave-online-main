...
import 'package:egyptm/app/pages/main_page/home_page/notification_page.dart';
import 'package:egyptm/app/pages/main_page/home_page/search_page/suggested_search_page.dart';
...
class HomeWidget{
  ...
  MainWidget.menuButton(
  AppAssets.qrSvg, // تأكد إن عندك SVG اسمه qrSvg في AppAssets
  false,
  Colors.white,
  Colors.black.withOpacity(.2),
  () async {
    ...
    if (scannedCode != null) {
      // استدعاء API بنفس طريقة EnrollmentPage
      final url = '${Constants.baseUrl}panel/use-code-course';
      bool response = await CourseService.enrollment(url, scannedCode);

      if (response) {
        nextRoute(MainPage.pageName);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('تم استخدام الكود بنجاح')),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('فشل في استخدام الكود')),
        );
      }
    }
  },
                                    ...
                                    // space(0,width: 12),
                                    // MainWidget.menuButton(
                                    //   AppAssets.orientationSvg,
                                    //   userProvider.notification.where((element) => element.status == 'unread').isNotEmpty,
                                    //   Colors.white,
                                    //   Colors.black.withOpacity(.2),
                                    //       () {
                                    //     if (isLandscape) {
                                    //       // تغيير الاتجاه إلى الوضع العمودي
