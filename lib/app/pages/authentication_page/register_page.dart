...
import 'package:egyptm/app/pages/authentication_page/login_page.dart';
import 'package:egyptm/app/pages/authentication_page/verify_code_page.dart';
import 'package:egyptm/app/pages/main_page/home_page/single_course_page/single_content_page/web_view_page.dart';
...
import 'package:egyptm/app/widgets/authentication_widget/auth_widget.dart';
import 'package:egyptm/app/widgets/authentication_widget/country_code_widget/code_country.dart';
import 'package:egyptm/app/widgets/authentication_widget/register_widget/register_widget.dart';
import 'package:egyptm/app/widgets/main_widget/main_widget.dart';
...
class _RegisterPageState extends State<RegisterPage> {
  ...
  String? otherRegisterMethod = 'email';
  ...
  void initState() {
    ...
    if((PublicData.apiConfigData?['register_method'] ?? '') == 'phone'){
      isPhoneNumber = false;
  ...
  Future<void> getCategoriesData() async {
    var carfromapi;
     carfromapi = await CategoriesService.categories(isRegister: true);
     List<CategoryModel> allCategories = carfromapi;
     categories = carfromapi;
    print(allCategories);
    ...
    setState(() {
      roles = allCategories
          .where((i) => i.title != null && i.id != null) // تأكد من عدم وجود null
          .map((i) => {
        'id': i.id,
        'title': i.title
      })
    ...
    // categories = await CategoriesService.categories(isRegister: true);
  ...
  String? getLastSelectedCategory() {
    if (selectedSubSubCategoryId != null) {
      return subSubCategories.firstWhere((sub) => sub.id.toString() == selectedSubSubCategoryId).title;
    } else if (selectedSubCategoryId != null) {
      return subCategories.firstWhere((sub) => sub.id.toString() == selectedSubCategoryId).title;
    } else if (selectedCategoryId != null) {
      return categories.firstWhere((cat) => cat.id.toString() == selectedCategoryId).title;
    }
    ...
    // Other Register Method
                      if(registerConfig?.showOtherRegisterMethod != null)...{
                        ...
                        Container(
                          ...
                          space(16),
Text(appText.profileImage, style: style14Regular().copyWith(color: greyB2)),
    ...
    child: selectedImage != null
        ...
        : Icon(Icons.camera_alt, color: Colors.grey),
                  ...
                  DropDownCustomTextfailed(
                    ...
                    hintText: appText.chooseCollege,
                    dropdownItems: categories.map((category) => category.title!).toList(),
                    onDropdownChanged: (value) {
                      setState(() {
                        // تحديد `categoryId`
                        selectedCategoryId = categories.firstWhere((category) => category.title == value).id.toString();
                        // تحديث القائمة الفرعية بالمستوى الثاني
                        subCategories = categories.firstWhere((category) => category.title == value).subCategories ?? [];
                  ...
                  if (subCategories.isNotEmpty)
                    DropDownCustomTextfailed(
                      prefixIcon: const Icon(Icons.arrow_drop_down),
                      hintText: appText.chooseLevel,
                      dropdownItems: subCategories.map((subCategory) => subCategory.title!).toList(),
                      onDropdownChanged: (value) {
                        setState(() {
                          // تحديد `subCategoryId`
                          selectedSubCategoryId = subCategories.firstWhere((subCategory) => subCategory.title == value).id.toString();
                          // تحديث القائمة الفرعية بالمستوى الثالث
                          subSubCategories = subCategories.firstWhere((subCategory) => subCategory.title == value).subCategories ?? [];
                  ...
                  if (subSubCategories.isNotEmpty)
                    DropDownCustomTextfailed(
                      ...
                      dropdownItems: subSubCategories.map((subSubCategory) => subSubCategory.title!).toList(),
                      onDropdownChanged: (value) {
                        setState(() {
                          // تحديد `subSubCategoryId`
                          selectedSubSubCategoryId = subSubCategories.firstWhere((subSubCategory) => subSubCategory.title == value).id.toString();
                        });
                      },
                    ),
